...
export default {
  ...
  methods: {
    handlerTreeSelected(nodeList, node) {
      ...
      nodeList
        .filter((n) => n.node.selected && n.nodeKey !== node.nodeKey)
        .map((n) => {
    ...
    handlerDragend(e, { root, node, data }) {
      ...
      const removeNode = () => {
        const parent = root.find(
          (el) => el.children && el.children.indexOf(data.nodeKey) > -1
        ).node;
      ...
      const doAppend = (isBefore) => {
        const parent = root.find(
          (el) => el.children && el.children.indexOf(APPEND_TARGET.nodeKey) > -1
        ).node;
        const index =
          parent.children.indexOf(APPEND_TARGET) + (isBefore ? 0 : 1);
