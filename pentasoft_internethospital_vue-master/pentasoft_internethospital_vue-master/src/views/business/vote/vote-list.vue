  ...
  <div>
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddVoteForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'vote-list-add'"
          >新建数据</Button
        >
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'vote-list-batch-delete'"
          >批量删除</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'vote-list-export-all'"
          >导出全部</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'vote-list-batch-export'"
          >批量导出</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.voteId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import VoteListForm from './components/vote-list-form';
import VoteItemListForm from './vote-item-list';
    ...
    return {
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          {
            title: '是否公开参与',
            key: 'isPublicParticipate',
            render: (h, params) => {
              return h(
                'span',
                this.$enum.getDescByValue(
                  'YES_OR_NO',
                  params.row.isPublicParticipate
                )
              );
            }
          },
          {
            title: '投票开始日期',
            key: 'voteBeginDate',
            tableColumn: 'ybj_votes.VoteBeginDate',
            sortable: 'custom'
          },
          {
            title: '投票结束日期',
            key: 'voteEndDate',
            tableColumn: 'ybj_votes.VoteEndDate',
            sortable: 'custom'
          },
          {
            title: '投票总数',
            key: 'voteTotal',
            tableColumn: 'ybj_votes.VoteTotal',
            sortable: 'custom'
          },
          {
            title: '操作',
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'vote-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditVoteForm(params.row);
                  }
                },
                {
                  title: '删除',
                  ...
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteVote(params.row.voteId);
                      }
                    });
                  }
                }
  ...
  methods: {
    ...
    async deleteVote (id) {
      try {
        ...
        let result = await voteApi.deleteVote(id);
    ...
    async batchDelete () {
      this.$Spin.show();
      await voteApi.batchDeleteVote(
        this.mainTableSelectArray.map((e) => e.voteId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await voteApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
