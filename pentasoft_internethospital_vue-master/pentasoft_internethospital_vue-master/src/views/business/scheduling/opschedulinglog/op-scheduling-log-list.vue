  ...
  <div>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="space-between" class="search-row">
        <Col span="12">
          <span class="title">门诊日志类型 :</span>
          <Input
            placeholder="请输入门诊日志类型"
            style="width: 180px"
            v-model="queryForm.oPSchedulingLogTypeId"
          />
        </Col>
        <Col span="12">
          ...
          <Input
            placeholder="请输入操作类型"
            style="width: 180px"
            v-model="queryForm.oPSchedulingOperateTypeId"
          />
        </Col>
      </Row>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'o-p-scheduling-log-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddOPSchedulingLogForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'o-p-scheduling-log-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'o-p-scheduling-log-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'o-p-scheduling-log-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'o-p-scheduling-log-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import OPSchedulingLogListForm from "./components/op-scheduling-log-list-form";
import { WX_OPSCHEDULINGLOG_TYPE } from "@/constants/enum-wx";
const PAGE_SIZE_INIT = 20;
export default {
  name: "OPSchedulingLogList",
  components: {
    OPSchedulingLogListForm,
  },
  props: {},
    ...
    return {
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          {
            title: "操作对象",
            key: "opSchedulingLogTypeId",
            tableColumn: "smart_OPSchedulingLogs.opSchedulingLogTypeId",
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue(
                  "WX_OPSCHEDULINGLOG_TYPE",
                  params.row.opschedulingLogTypeId
                )
              );
            },
          },
          {
            title: "操作类型",
            key: "opschedulingOperateTypeId",
            tableColumn: "smart_OPSchedulingLogs.opschedulingOperateTypeId",
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue(
                  "WX_OPSCHEDULINGOPRATE_TYPE",
                  params.row.opschedulingOperateTypeId
                )
              );
            },
          },
          {
            title: "操作记录",
            key: "objectModelDescription",
            tableColumn: "smart_OPSchedulingLogs.ObjectModelDescription",
            sortable: "custom",
          },
          {
            title: "操作",
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "o-p-scheduling-log-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditOPSchedulingLogForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "o-p-scheduling-log-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteOPSchedulingLog(
                          params.row.opschedulingLogId
                        );
                      },
                    });
                  },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await oPSchedulingLogApi.queryOPSchedulingLog(params);
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteOPSchedulingLog(oPSchedulingLogId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await oPSchedulingLogApi.deleteOPSchedulingLog(
          oPSchedulingLogId
        );
    ...
    async batchDelete() {
      this.$Spin.show();
      await oPSchedulingLogApi.batchDeleteOPSchedulingLog(
        this.mainTableSelectArray.map((e) => e.opschedulingLogId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await oPSchedulingLogApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await oPSchedulingLogApi.batchExport(
          this.mainTableSelectArray.map((e) => e.oPSchedulingLogId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddOPSchedulingLogForm() {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditOPSchedulingLogForm(updateObject) {
