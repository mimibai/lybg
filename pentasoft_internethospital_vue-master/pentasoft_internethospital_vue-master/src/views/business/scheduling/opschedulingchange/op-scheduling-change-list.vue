  ...
  <div>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'op-scheduling-change-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddOpSchedulingChangeForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'op-scheduling-change-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'op-scheduling-change-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'op-scheduling-change-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'op-scheduling-change-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import OpSchedulingChangeListForm from "./components/op-scheduling-change-list-form";
import { YES_OR_NO } from "@/constants/common";
const PAGE_SIZE_INIT = 20;
export default {
  name: "OpSchedulingChangeList",
  components: {
    OpSchedulingChangeListForm,
  },
  props: {},
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            title: "名称",
            key: "name",
            tableColumn: "smart_OPSchedulingChanges.Name",
            sortable: "custom",
          },
          {
            title: "操作日期",
            key: "operationDateTime",
            tableColumn: "smart_OPSchedulingChanges.OperationDateTime",
            sortable: "custom",
          },
          {
            title: "换诊日期",
            key: "changeRestDateTime",
            tableColumn: "smart_OPSchedulingChanges.ChangeRestDateTime",
            sortable: "custom",
          },
          {
            title: "是否启用",
          ...
          },
          {
            title: "备注",
            key: "reMark",
            tableColumn: "smart_OPSchedulingChanges.ReMark",
            sortable: "custom",
          },
          {
            title: "是否删除",
          ...
          {
            title: "是否清空换诊",
            key: "isClearChangeScheduling",
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue(
                  "YES_OR_NO",
                  params.row.isClearChangeScheduling
                )
              );
            },
          },
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "op-scheduling-change-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditOpSchedulingChangeForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "op-scheduling-change-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteOpSchedulingChange(
                          params.row.opschedulingChangeId
                        );
                      },
                    });
                  },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await opSchedulingChangeApi.queryOpSchedulingChange(
          params
        );
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteOpSchedulingChange(opSchedulingChangeId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await opSchedulingChangeApi.deleteOpSchedulingChange(
    ...
    async batchDelete() {
      this.$Spin.show();
      await opSchedulingChangeApi.batchDeleteOpSchedulingChange(
        this.mainTableSelectArray.map((e) => e.opschedulingChangeId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await opSchedulingChangeApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await opSchedulingChangeApi.batchExport(
          this.mainTableSelectArray.map((e) => e.opSchedulingChangeId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddOpSchedulingChangeForm() {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditOpSchedulingChangeForm(updateObject) {
