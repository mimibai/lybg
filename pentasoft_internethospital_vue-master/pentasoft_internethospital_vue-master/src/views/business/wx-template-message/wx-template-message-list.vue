  ...
  <div>
      ...
      <Row class="smart-query-form-row">
        ...
        <span>
          ...
          <Input
            ...
            style="width: 180px"
            v-model="queryForm.appWeiXinMessageTemplateId"
          />
        </span>
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'wx-template-message-list-query'"
            >查询</Button
          >
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddWeixintemplatemessageForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'wx-template-message-list-add'"
          >新建数据</Button
        >
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'wx-template-message-list-batch-delete'"
          >批量删除</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.weiXinTemplateMessageId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import WxTemplateMessageListForm from "./components/wx-template-message-list-form";
    ...
    return {
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          {
            title: "消息模板",
            key: "appWeiXinMessageTemplateId",
            tableColumn:
              "ybj_weixintemplatemessages.AppWeiXinMessageTemplateId",
            sortable: "custom",
          },
          {
            title: "发送状态",
            key: "weiXinTemplateMessageStatuTypeId",
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue(
                  "WX_TEMPLATE_MESSAGE_STATU_TYPES",
                  params.row.weiXinTemplateMessageStatuTypeId
                )
              );
            },
          },
          {
            title: "微信返回消息id",
            key: "msgID",
            tableColumn: "ybj_weixintemplatemessages.msgID",
            sortable: "custom",
          },
          {
            title: "备注",
            key: "reMark",
            tableColumn: "ybj_weixintemplatemessages.reMark",
            sortable: "custom",
          },
          {
            title: "是否删除",
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "wx-template-message-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditWeixintemplatemessageForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "wx-template-message-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteWeixintemplatemessage(params.row.weiXinTemplateMessageId);
                      },
                    });
                  },
                },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await wxTemplateMessageApi.queryWeixintemplatemessage(
          params
        );
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteWeixintemplatemessage(weiXinTemplateMessageId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await wxTemplateMessageApi.deleteWeixintemplatemessageById(
          weiXinTemplateMessageId
        );
    ...
    async batchDelete() {
      this.$Spin.show();
      await wxTemplateMessageApi.batchDeleteWeixintemplatemessage(
        this.mainTableSelectArray.map((e) => e.weiXinTemplateMessageId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await wxTemplateMessageApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await wxTemplateMessageApi.batchExport(
          this.mainTableSelectArray.map((e) => e.weiXinTemplateMessageId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddWeixintemplatemessageForm() {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditWeixintemplatemessageForm(updateObject) {
