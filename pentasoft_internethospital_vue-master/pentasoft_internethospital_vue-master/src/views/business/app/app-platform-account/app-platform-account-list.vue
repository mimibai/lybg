  ...
  <div>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="space-between" class="search-row">
        ...
        <Col span="8">
          <span>是否启用 :</span>
          <Checkbox v-model="queryForm.isOpen"></Checkbox>
        </Col>
      </Row>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'app-platform-account-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddAppPlatformAccountForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'app-platform-account-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'app-platform-account-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'app-platform-account-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'app-platform-account-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import AppPlatformAccountListForm from './components/app-platform-account-list-form';
        ...
        columnArray: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h(
                'span',
                this.$enum.getDescByValue('YES_OR_NO', params.row.isOpen)
              );
            }
          },
          {
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('span', params.row.schedulingSource ? '出诊数据' : '在线问诊');
            }
          },
          {
            title: '即时通信IM配置',
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'app-platform-account-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditAppPlatformAccountForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'app-platform-account-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteAppPlatformAccount(
                          params.row.appPlatformAccountId
                        );
                      }
                    });
                  }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result = await appPlatformAccountApi.queryAppPlatformAccount(
          params
        );
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteAppPlatformAccount (appPlatformAccountId) {
      try {
        this.isShowDeleteLoading = true;
        let result = await appPlatformAccountApi.deleteAppPlatformAccount(
    ...
    async batchDelete () {
      this.$Spin.show();
      await appPlatformAccountApi.batchDeleteAppPlatformAccount(
        this.mainTableSelectArray.map((e) => e.appPlatformAccountId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await appPlatformAccountApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await appPlatformAccountApi.batchExport(
          this.mainTableSelectArray.map((e) => e.appPlatformAccountId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddAppPlatformAccountForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditAppPlatformAccountForm (updateObject) {
