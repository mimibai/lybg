  ...
  <div>
      ...
      <Row class="smart-query-form-row">
        ...
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'app-wx-message-template-list-query'"
            >查询</Button
          >
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddAppweixinmessagetemplateForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'app-wx-message-template-list-add'"
          >新建数据</Button
        >
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'app-wx-message-template-list-batch-delete'"
          >批量删除</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.appWeiXinMessageTemplateId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import AppWxMessageTemplateListForm from "./components/app-wx-message-template-list-form";
import { APP_WX_MESSAGE_TEMPLATE_TYPES } from "@/constants/enum-wx";
import { wxAccountApi } from "@/api/weixin/wx-account";
const PAGE_SIZE_INIT = 20;
export default {
  name: "AppWxMessageTemplateList",
  components: {
    AppWxMessageTemplateListForm,
  },
  props: {},
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            title: "标题",
            key: "name",
            tableColumn: "ybj_appweixinmessagetemplates.Name",
            sortable: "custom",
          },
          {
            title: "模板类型",
            key: "appWeiXinMessageTemplateTypeId",
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue(
                  "APP_WX_MESSAGE_TEMPLATE_TYPES",
                  params.row.appWeiXinMessageTemplateTypeId
                )
              );
            },
          },
          {
            title: "模板编号",
            key: "templateIdShort",
            tableColumn: "ybj_appweixinmessagetemplates.TemplateIdShort",
            sortable: "custom",
          },
          {
            title: "模板ID",
            key: "templateId",
            tableColumn: "ybj_appweixinmessagetemplates.TemplateId",
            sortable: "custom",
          },
          {
            title: "总发送数",
            key: "totalSendCount",
            tableColumn: "ybj_appweixinmessagetemplates.TotalSendCount",
            sortable: "custom",
          },
          {
            title: "月发送数",
            key: "monthSendCount",
            tableColumn: "ybj_appweixinmessagetemplates.MonthSendCount",
            sortable: "custom",
          },
          {
            title: "是否可用",
          ...
          },
          {
            title: "备注",
            key: "reMark",
            tableColumn: "ybj_appweixinmessagetemplates.ReMark",
            sortable: "custom",
          },
          {
            title: "是否删除",
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "app-wx-message-template-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditAppweixinmessagetemplateForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "app-wx-message-template-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteAppweixinmessagetemplate(params.row.appWeiXinMessageTemplateId);
                      },
                    });
                  },
                },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await appWxMessageTemplateApi.queryAppweixinmessagetemplate(
          params
        );
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteAppweixinmessagetemplate(appWeiXinMessageTemplateId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await appWxMessageTemplateApi.deleteAppweixinmessagetemplate(
    ...
    async batchDelete() {
      this.$Spin.show();
      await appWxMessageTemplateApi.batchDeleteAppweixinmessagetemplate(
        this.mainTableSelectArray.map((e) => e.appWeiXinMessageTemplateId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await appWxMessageTemplateApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await appWxMessageTemplateApi.batchExport(
          this.mainTableSelectArray.map((e) => e.appWeiXinMessageTemplateId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddAppweixinmessagetemplateForm() {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditAppweixinmessagetemplateForm(updateObject) {
