  ...
  <div>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="space-between" class="search-row">
        ...
        <Col span="8">
          <span>图片类型 :</span>
          <EnumSelect v-model="queryForm.imageTypeId" style="width: 180px" code="ImageTypeEnum" />
        </Col>
        <Col span="8">
          <span>是否启用 :</span>
          <i-switch v-model="queryForm.isAvailable" />
        </Col>
        <Col span="8">
          <span>是否是精灵图 :</span>
          <i-switch v-model="queryForm.isWizardMap" />
        </Col>
      </Row>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'app-image-link-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddAppImageLinkForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'app-image-link-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'app-image-link-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'app-image-link-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'app-image-link-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import AppImageLinkListForm from './components/app-image-link-list-form';
...
export default {
        ...
        columnArray: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h(
                'span',
                this.$enum.getDescByValue('YES_OR_NO', params.row.isWizardMap)
              );
            }
          },
          {
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'app-image-link-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditAppImageLinkForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'app-image-link-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteAppImageLink(params.row.appImageLinkId);
                      }
                    });
                  }
                }
  ...
  methods: {
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteAppImageLink (appImageLinkId) {
      try {
        this.isShowDeleteLoading = true;
        let result = await appImageLinkApi.deleteAppImageLink(appImageLinkId);
    ...
    async batchDelete () {
      this.$Spin.show();
      await appImageLinkApi.batchDeleteAppImageLink(
        this.mainTableSelectArray.map((e) => e.appImageLinkId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await appImageLinkApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await appImageLinkApi.batchExport(
          this.mainTableSelectArray.map((e) => e.appImageLinkId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddAppImageLinkForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditAppImageLinkForm (updateObject) {
