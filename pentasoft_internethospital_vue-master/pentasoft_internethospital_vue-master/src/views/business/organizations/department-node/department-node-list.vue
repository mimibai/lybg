<template>
  <div>
    <Tabs type="card" value="name1" @on-click="changeTabs">
      <TabPane label="通用科室" name="name1"> </TabPane>
      <TabPane label="特色科室" name="name2"> </TabPane>
    </Tabs>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="start" class="search-row">
        ...
        <Col span="8">
          <span class="title"> 是否在首页隐藏 : </span>
          <Select
            placeholder="请选择"
            style="width: 200px"
            v-model="queryForm.isHideInHome"
          >
        ...
        <Col span="8">
          <span class="title"> 是否显示在手机端 : </span>
          <Select
            placeholder="请选择"
            style="width: 200px"
            v-model="queryForm.isShowInMobile"
          >
            <Option value>不限</Option>
            <Option
              :key="item.value"
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'department-node-list-query'"
            >查询</Button
          >
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddDepartmentNodeForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'department-node-list-add'"
          >新建数据</Button
        >
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'department-node-list-batch-delete'"
          >批量删除</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'department-node-list-export-all'"
          >导出全部</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'department-node-list-batch-export'"
          >批量导出</Button
        >
      </Row>
      <!-------操作按钮行 end------->
    ...
    <Modal
      ...
      width="60%"
    >
      <DepartmentNodeListForm
        :isUpdate="saveModal.isUpdate"
        :updateData="saveModal.updateData"
        :categoryId="saveModal.departmentNodeCategoryId"
...
import DepartmentNodeListForm from "./components/department-node-list-form";
...
export default {
        ...
        columnArray: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue("YES_OR_NO", params.row.isHideInHome)
              );
            },
          },
          {
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "department-node-list-move",
                    },
                  ],
                  action: () => {
                    this.moveDepartmentNode(params.row.departmentNodeId, "up");
                  },
                },
                {
                  title: "下移",
                  ...
                  action: () => {
                    this.moveDepartmentNode(
                      params.row.departmentNodeId,
                      "down"
                    );
                  },
                },
                ...
                //   ],
                //   action: () => {
                //     this.moveDepartmentNode(params.row.departmentNodeId, "upgrade");
                //   },
                // },
                // {
                //   title: "降级",
                ...
                //   ],
                //   action: () => {
                //     this.moveDepartmentNode(params.row.departmentNodeId, "downgrade");
                //   },
                // },
                {
                  title: "添加子节点",
                  directives: [
                    {
                      name: "privilege",
                      value: "department-node-list-add",
                    },
                  ],
                  action: () => {
                    this.showAddDepartmentNodeForm(params.row.departmentNodeId);
                  },
                },
                {
                  title: "编辑",
                  directives: [
                    {
                      name: "privilege",
                      value: "department-node-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditDepartmentNodeForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "department-node-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteDepartmentNode(params.row.departmentNodeId);
                      },
                    });
                  },
                },
                // {
                //   title: (params.row.site.isOffline||false) ? "启用站点" : "关闭站点",
                //   directives: [
                //     {
                //       name: "privilege",
                //       value: "department-node-list-close",
                //     },
                //   ],
                //   action: () => {
                //     this.$Modal.confirm({
                //       title: "友情提醒",
                //       content: "你确定要修改此站点的开启状态吗?",
                //       onOk: () => {
                //         this.closeDepartmentNode(params.row.departmentNodeId);
                //       },
                //     });
                //   },
                // },
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "doctor-list-query",
                    },
                  ],
                  action: () => {
                    this.$router.push({
                      path: "/departmentNode/department-releated-doctor-list",
                      query: { departmentNodeId: params.row.departmentNodeId },
                    });
                  },
                },
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "disease-list-query",
                    },
                  ],
                  action: () => {
                    this.$router.push({
                      name: "DiseaseList",
                      params: { departmentNodeId: 123 },
                    });
                  },
                },
              ];
  ...
  methods: {
    changeTabs(name) {
      if (name == "name2") {
        this.queryForm.departmentNodeCategoryId = 2;
      } else {
        this.queryForm.departmentNodeCategoryId = 1;
      }
      this.queryList();
    },
    /* -------------------------查询相关 begin------------------------- */
    ...
    /*-------------------------节点移动 begin------------------------- */
    async moveDepartmentNode(departmentNodeId, direction) {
      try {
        this.isShowMoveLoading = true;
        let result = await departmentNodeApi.moveDepartmentNode(
          departmentNodeId,
          direction
        );
    ...
    convertToTreeData(list, dataArr) {
      dataArr.map((pNode, i) => {
        let childObj = [];
        list.map((cNode, j) => {
          if (pNode.departmentNodeId === cNode.parentId) {
            childObj.push(cNode);
          }
        });
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteDepartmentNode(departmentNodeId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await departmentNodeApi.deleteDepartmentNode(
          departmentNodeId
        );
    ...
    async batchDelete() {
      this.$Spin.show();
      await departmentNodeApi.batchDeleteDepartmentNode(
        this.mainTableSelectArray.map((e) => e.departmentNodeId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await departmentNodeApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await departmentNodeApi.batchExport(
          this.mainTableSelectArray.map((e) => e.departmentNodeId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddDepartmentNodeForm(parentId) {
      this.saveModal.parentId = parentId || 0;
      this.saveModal.departmentNodeCategoryId =
        this.queryForm.departmentNodeCategoryId;
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditDepartmentNodeForm(updateObject) {
      this.saveModal.departmentNodeCategoryId =
        this.queryForm.departmentNodeCategoryId;
