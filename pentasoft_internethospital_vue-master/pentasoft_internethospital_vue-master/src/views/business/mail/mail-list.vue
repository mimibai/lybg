      ...
      <Row class="smart-query-form-row">
        ...
        <span>
          ...
          <Input
            placeholder="请输入是否回复"
            style="width: 180px"
            v-model="queryForm.isReply"
          />
        </span>
        <span>
          是否为匿名 :
          <Input
            placeholder="请输入是否为匿名"
            style="width: 180px"
            v-model="queryForm.isAnonymous"
          />
        </span>
        <span>
          是否为公开 :
          <Input
            placeholder="请输入是否为公开"
            style="width: 180px"
            v-model="queryForm.isPublic"
          />
        </span>
        <span>
          邮件类型Id :
          <Input
            placeholder="请输入邮件类型Id"
            style="width: 180px"
            v-model="queryForm.mailLabelId"
          />
        </span>
        <span>
          是否回复 :
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        ...
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'mail-list-batch-delete'"
          >批量删除</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'mail-list-export-all'"
          >导出全部</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'mail-list-batch-export'"
          >批量导出</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.mailId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import MailListForm from "./components/mail-list-form";
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },         
          {
            title: "被举报对象",
            key: "title",
            tableColumn: "ybj_mails.Title",
            sortable: "custom",
          },
          {
            title: "举报人姓名",
            key: "sendRealName",
            tableColumn: "ybj_mails.SendRealName",
            sortable: "custom",
          },
          {
            title: "举报人联系方式",
            key: "otherContacts",
            tableColumn: "ybj_mails.OtherContacts",
            sortable: "custom",
          },
          {
            title: "提问时间",
            key: "create_time",
            tableColumn: "ybj_mails.create_time",
            sortable: "custom",
          },
         
          {
            title: "医院信箱类型Id",
            key: "mailTypeId",
            tableColumn: "ybj_mails.MailTypeId",
            sortable: "custom",
          },
          {
            title: "是否回复",
          ...
          },          
          {
            ...
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue("YES_OR_NO", params.row.isPublic)
              );
            },
          },
          {
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "mail-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditMailForm(params.row);
                  },
                },
                {
                  title: "删除",
                  ...
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteMail(params.row.mailId);
                      },
                    });
                  },
                },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await mailApi.queryMail(params);
    ...
    async deleteMail(mailId) {
      try {
        ...
        let result = await mailApi.deleteMail(mailId);
    ...
    async batchDelete() {
      this.$Spin.show();
      await mailApi.batchDeleteMail(this.mainTableSelectArray.map((e) => e.mailId));
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await mailApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await mailApi.batchExport(this.mainTableSelectArray.map((e) => e.mailId));
      } catch (e) {
        console.log(e);
      } finally {
