<template>
  <div>
    <Form ref="form" :rules="formValidate" :label-width="90" :model="form">
      <FormItem label="主键" prop="locationBranchId">
        <Input type="number" v-model.number="form.locationBranchId" />
      </FormItem>
      <FormItem label="院区名称" prop="locationBranchName">
        <Input v-model="form.locationBranchName" />
      </FormItem>
...
export default {
  ...
  watch: {
    updateData: function (newValue, oldValue) {
      ...
      if (this.isUpdate) {
        ...
        this.$set(this.form, "locationBranchId", newValue["locationBranchId"]);
      }
    },
  ...
  methods: {
    ...
    async add() {
      this.$Spin.show();
      let res = await locationBranchApi.addLocationBranch(this.form);
    ...
    async update() {
      this.$Spin.show();
      let res = await locationBranchApi.updateLocationBranch(this.form);
      this.$Message.success(res.msg);
      this.$Spin.hide();
      this.resetForm();
      this.$emit("on-form-close");
