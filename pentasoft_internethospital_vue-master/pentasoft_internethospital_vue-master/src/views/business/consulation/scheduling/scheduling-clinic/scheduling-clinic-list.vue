  ...
  <div>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'scheduling-clinic-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddSchedulingClinicForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'scheduling-clinic-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'scheduling-clinic-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'scheduling-clinic-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'scheduling-clinic-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import SchedulingClinicListForm from './components/scheduling-clinic-list-form';

const PAGE_SIZE_INIT = 20;
export default {
  name: 'SchedulingClinicList',
  components: {
    SchedulingClinicListForm
  },
  props: {},
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            ...
            tableColumn: 'smart_SchedulingClinics.SchedulingClinicId',
            sortable: 'custom'
          },
          {
            title: '名称',
            key: 'name',
            tableColumn: 'smart_SchedulingClinics.Name',
            sortable: 'custom'
          },
          {
            title: '简称',
            key: 'shortName',
            tableColumn: 'smart_SchedulingClinics.ShortName',
            sortable: 'custom'
          },
          {
            title: '描述',
            key: 'description',
            tableColumn: 'smart_SchedulingClinics.Description',
            sortable: 'custom'
          },
          {
            title: '排序',
            key: 'sorting',
            tableColumn: 'smart_SchedulingClinics.Sorting',
            sortable: 'custom'
          },
          {
            title: '创建日期',
            key: 'dateCreated',
            tableColumn: 'smart_SchedulingClinics.DateCreated',
            sortable: 'custom'
          },
          {
            title: '修改日期',
            key: 'dateModified',
            tableColumn: 'smart_SchedulingClinics.DateModified',
            sortable: 'custom'
          },
          {
            title: '编码',
            key: 'code',
            tableColumn: 'smart_SchedulingClinics.Code',
            sortable: 'custom'
          },
          {
            title: '适用科室Id集合',
            key: 'departmentIds',
            tableColumn: 'smart_SchedulingClinics.DepartmentIds',
            sortable: 'custom'
          },
          {
            title: '操作',
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'scheduling-clinic-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditSchedulingClinicForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'scheduling-clinic-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteSchedulingClinic(
                          params.row.schedulingClinicId
                        );
                      }
                    });
                  }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result = await schedulingClinicApi.querySchedulingClinic(params);
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteSchedulingClinic (schedulingClinicId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await schedulingClinicApi.deleteSchedulingClinic(
          schedulingClinicId
        );
    ...
    async batchDelete () {
      this.$Spin.show();
      await schedulingClinicApi.batchDeleteSchedulingClinic(
        this.mainTableSelectArray.map((e) => e.schedulingClinicId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await schedulingClinicApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await schedulingClinicApi.batchExport(
          this.mainTableSelectArray.map((e) => e.schedulingClinicId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddSchedulingClinicForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditSchedulingClinicForm (updateObject) {
