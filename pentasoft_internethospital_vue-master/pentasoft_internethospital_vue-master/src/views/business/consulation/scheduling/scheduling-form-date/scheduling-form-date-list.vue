  ...
  <div>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="space-between" class="search-row">
        <Col span="8">
          <span>出诊表单Id :</span>
          <Input
            placeholder="请输入出诊表单Id"
            style="width: 180px"
            v-model="queryForm.schedulingFormId"
          />
        </Col>
        <Col span="8">
          <span>显示名称 :</span>
        ...
        <Col span="8">
          <span>门诊专业Id集合 :</span>
          <Input
            placeholder="请输入门诊专业Id集合"
            style="width: 180px"
            v-model="queryForm.schedulingSpecialtyIds"
          />
        </Col>
        <Col span="8">
          <span>出诊级别Id集合 :</span>
          <Input
            placeholder="请输入出诊级别Id集合"
            style="width: 180px"
            v-model="queryForm.schedulingLevelIds"
          />
        </Col>
        <Col span="8">
          <span>医疗职称Id集合 :</span>
          <Input
            placeholder="请输入医疗职称Id集合"
            style="width: 180px"
            v-model="queryForm.professionalTitleIds"
          />
        </Col>
        <Col span="8">
          <span>是否显示数据 :</span>
          <Input
            placeholder="请输入是否显示数据"
            style="width: 180px"
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'scheduling-form-date-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddSchedulingFormDateForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'scheduling-form-date-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'scheduling-form-date-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'scheduling-form-date-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'scheduling-form-date-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import SchedulingFormDateListForm from './components/scheduling-form-date-list-form';
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            title: '排序',
            key: 'sorting',
            tableColumn: 'smart_SchedulingFormDates.Sorting',
            sortable: 'custom'
          },
          {
            title: '创建日期',
            key: 'dateCreated',
            tableColumn: 'smart_SchedulingFormDates.DateCreated',
            sortable: 'custom'
          },
          {
            title: '修改日期',
            key: 'dateModified',
            tableColumn: 'smart_SchedulingFormDates.DateModified',
            sortable: 'custom'
          },
          {
            title: '操作',
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'scheduling-form-date-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditSchedulingFormDateForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'scheduling-form-date-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteSchedulingFormDate(
                          params.row.schedulingFormDateId
                        );
                      }
                    });
                  }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result = await schedulingFormDateApi.querySchedulingFormDate(
          params
        );
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteSchedulingFormDate (schedulingFormDateId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await schedulingFormDateApi.deleteSchedulingFormDate(
          schedulingFormDateId
        );
    ...
    async batchDelete () {
      this.$Spin.show();
      await schedulingFormDateApi.batchDeleteSchedulingFormDate(
        this.mainTableSelectArray.map((e) => e.schedulingFormDateId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await schedulingFormDateApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await schedulingFormDateApi.batchExport(
          this.mainTableSelectArray.map((e) => e.schedulingFormDateId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddSchedulingFormDateForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditSchedulingFormDateForm (updateObject) {
