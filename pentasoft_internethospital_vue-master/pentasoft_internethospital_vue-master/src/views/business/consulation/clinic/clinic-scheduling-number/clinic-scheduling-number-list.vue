  ...
  <div>
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddClinicSchedulingNumberForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'clinic-scheduling-number-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'clinic-scheduling-number-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'clinic-scheduling-number-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'clinic-scheduling-number-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
          ...
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li
                ...
                :key="item.clinicSchedulingNumberId"
              >
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
    ...
    <Modal
      ...
      width="60%"
    >
      <ClinicSchedulingNumberListForm
        :isUpdate="saveModal.isUpdate"
        :updateData="saveModal.updateData"
        :clinicSchedulingId="clinicSchedulingId"
...
import ClinicSchedulingNumberListForm from './components/clinic-scheduling-number-list-form';
    ...
    return {
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          {
            title: '号源生成记录',
            key: 'clinicSchedulingNumberGenerateLogId'
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'clinic-scheduling-number-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditClinicSchedulingNumberForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'clinic-scheduling-number-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteClinicSchedulingNumber(
                          params.row.clinicSchedulingNumberId
                        );
                      }
                    });
                  }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result =
          await clinicSchedulingNumberApi.queryClinicSchedulingNumber(params);
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteClinicSchedulingNumber (clinicSchedulingNumberId) {
      try {
        this.isShowDeleteLoading = true;
        let result =
          await clinicSchedulingNumberApi.deleteClinicSchedulingNumber(
    ...
    async batchDelete () {
      this.$Spin.show();
      await clinicSchedulingNumberApi.batchDeleteClinicSchedulingNumber(
        this.mainTableSelectArray.map((e) => e.clinicSchedulingNumberId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await clinicSchedulingNumberApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await clinicSchedulingNumberApi.batchExport(
          this.mainTableSelectArray.map((e) => e.clinicSchedulingNumberId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddClinicSchedulingNumberForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditClinicSchedulingNumberForm (updateObject) {
