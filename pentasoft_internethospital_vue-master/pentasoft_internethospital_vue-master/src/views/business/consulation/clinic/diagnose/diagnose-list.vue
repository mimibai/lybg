  ...
  <div>
    <Card class="smart-query-Card">
      ...
      >
        ...
        <Col span="8">
          <span>诊断编码类型 :</span>
          <Input
            placeholder="请输入诊断编码类型"
            style="width: 180px"
            v-model="queryForm.diagnoseCodeTypeId"
          />
        </Col>
        <Col span="8">
          <span>名称首字母 :</span>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'diagnose-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddDiagnoseForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'diagnose-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'diagnose-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'diagnose-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'diagnose-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.diagnoseId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import DiagnoseListForm from './components/diagnose-list-form';
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'diagnose-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditDiagnoseForm(params.row);
                  }
                },
                {
                  title: '删除',
                  ...
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteDiagnose(params.row.diagnoseId);
                      }
                    });
                  }
                }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result = await diagnoseApi.queryDiagnose(params);
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteDiagnose (diagnoseId) {
      try {
        this.isShowDeleteLoading = true;
        let result = await diagnoseApi.deleteDiagnose(diagnoseId);
    ...
    async batchDelete () {
      this.$Spin.show();
      await diagnoseApi.batchDeleteDiagnose(
        this.mainTableSelectArray.map((e) => e.diagnoseId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await diagnoseApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    showEditDiagnoseForm (updateObject) {
