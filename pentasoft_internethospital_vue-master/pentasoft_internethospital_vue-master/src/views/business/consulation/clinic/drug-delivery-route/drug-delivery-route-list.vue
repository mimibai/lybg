  ...
  <div>
    <Card class="smart-query-Card">
      ...
      <Row type="flex" justify="space-between" class="search-row">
        <Col span="8">
          ...
          <Input
            placeholder="请输入主键"
            style="width: 180px"
            v-model="queryForm.drugDeliveryRouteId"
          />
        </Col>
        <Col span="8">
          <span>名称 :</span>
      ...
      >
        <Col span="8">
          <span>药品限制类型 :</span>
          <Input
            placeholder="请输入药品限制类型"
            style="width: 180px"
            v-model="queryForm.drugTypeLimitId"
          />
        </Col>
        <Col span="8">
          <span>是否删除 :</span>
        ...
        <Col span="8">
          <span>药品剂型限制类型 :</span>
          <Input
            placeholder="请输入药品剂型限制类型"
            style="width: 180px"
            v-model="queryForm.drugDosageFormTypeLimitId"
          />
        </Col>
        <Col span="8">
          <span>创建时间:</span>
      ...
      <Row type="flex" justify="center">
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'drug-delivery-route-list-query'"
            >查询
          </Button>
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddDrugDeliveryRouteForm"
          icon="md-add"
          size="small"
          type="primary"
          v-privilege="'drug-delivery-route-list-add'"
          >新建数据
        </Button>
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'drug-delivery-route-list-batch-delete'"
          >批量删除
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'drug-delivery-route-list-export-all'"
          >导出全部
        </Button>
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'drug-delivery-route-list-batch-export'"
          >批量导出
        </Button>
      </Row>
      <!-------操作按钮行 end------->
...
import DrugDeliveryRouteListForm from './components/drug-delivery-route-list-form';
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: 'privilege',
                      value: 'drug-delivery-route-list-update'
                    }
                  ],
                  action: () => {
                    this.showEditDrugDeliveryRouteForm(params.row);
                  }
                },
                {
                  title: '删除',
                  directives: [
                    {
                      name: 'privilege',
                      value: 'drug-delivery-route-list-delete'
                    }
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: '确定要删除吗？',
                      onOk: () => {
                        this.deleteDrugDeliveryRoute(
                          params.row.drugDeliveryRouteId
                        );
                      }
                    });
                  }
  ...
  methods: {
    ...
    async queryList () {
      ...
      try {
        ...
        let result = await drugDeliveryRouteApi.queryDrugDeliveryRoute(params);
    ...
    /* -------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteDrugDeliveryRoute (drugDeliveryRouteId) {
      try {
        this.isShowDeleteLoading = true;
        let result = await drugDeliveryRouteApi.deleteDrugDeliveryRoute(
          drugDeliveryRouteId
        );
    ...
    async batchDelete () {
      this.$Spin.show();
      await drugDeliveryRouteApi.batchDeleteDrugDeliveryRoute(
        this.mainTableSelectArray.map((e) => e.drugDeliveryRouteId)
      );
    ...
    async exportAll () {
      try {
        ...
        let params = this.convertQueryParam();
        await drugDeliveryRouteApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport () {
      ...
      try {
        ...
        await drugDeliveryRouteApi.batchExport(
          this.mainTableSelectArray.map((e) => e.drugDeliveryRouteId)
        );
      } catch (e) {
        console.log(e);
    ...
    /* -------------------------添加，修改 表单 begin------------------------- */
    // 显示添加表单
    showAddDrugDeliveryRouteForm () {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditDrugDeliveryRouteForm (updateObject) {
