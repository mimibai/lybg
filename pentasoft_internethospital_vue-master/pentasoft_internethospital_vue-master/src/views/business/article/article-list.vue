      ...
      <Row class="smart-query-form-row">
        ...
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'article-list-query'"
            >查询</Button
          >
          <Button
    ...
    <Card class="warp-card">
      ...
      <Row class="marginBottom10">
        ...
        <Button
          ...
          size="small"
          type="error"
          v-privilege="'article-list-batch-delete'"
          >批量删除</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'article-list-export-all'"
          >导出全部</Button
        >
        ...
        <Button
          ...
          size="small"
          type="warning"
          v-privilege="'article-list-batch-export'"
          >批量导出</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.articleId">
                <a href="#">{{ item.title }}</a>
              </li>
            </ul>
          </Card>
...
import ArticleListForm from "./components/article-list-form";
        ...
        columnArray: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue("YES_OR_NO", params.row.isApprove)
              );
            },
          },
          {
            ...
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue("YES_OR_NO", params.row.isRelease)
              );
            },
          },
          {
            ...
            render: (h, params) => {
              return h(
                "span",
                this.$enum.getDescByValue("YES_OR_NO", params.row.isTop)
              );
            },
          },
          {
            title: "发布日期",
            key: "releaseDate",
            tableColumn: "ybj_articles.releaseDate",
            sortable: "custom",
          },
          {
            title: "浏览量",
            key: "viewCount",
            tableColumn: "ybj_articles.viewCount",
          },
          {
            title: "所属栏目",
            key: "columnName",
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "article-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditArticleForm(params.row);
                  },
                },
                {
                  title: "删除",
                  ...
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteArticle(params.row.articleId);
                      },
                    });
                  },
                },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await articleApi.queryArticle(params);
    ...
    async deleteArticle(articleId) {
      try {
        ...
        let result = await articleApi.deleteArticleById(articleId);
    ...
    async batchDelete() {
      this.$Spin.show();
      await articleApi.batchDeleteArticle(
        this.mainTableSelectArray.map((e) => e.articleId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await articleApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    showEditArticleForm(updateObject) {
