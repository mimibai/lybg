  ...
  <div>
      ...
      <Row class="smart-query-form-row">
        ...
        <ButtonGroup>
          <Button
            ...
            icon="ios-search"
            type="primary"
            v-privilege="'wx-bulk-message-list-query'"
            >查询</Button
          >
          <Button
    ...
    <Card class="warp-Card">
      ...
      <Row class="marginBottom10">
        <Button
          @click="showAddWeixinbulkmessageForm"
          icon="md-add"
          size="small"
          type="primary"
          class="float-right"
          v-privilege="'wx-bulk-message-list-add'"
          >群发</Button
        >
      </Row>
      <!-------操作按钮行 end------->
      ...
      <Form :label-width="80">
        ...
        <FormItem label="删除数据">
          <Card style="width: 350px; height: 250px; overflow-y: scroll">
            <ul>
              <li v-for="item in mainTableSelectArray" :key="item.weiXinBulkMessageId">
                <a href="#">{{ item.name }}</a>
              </li>
            </ul>
          </Card>
...
import WxBulkMessageListForm from "./components/wx-bulk-message-list-form";
import { wxAccountApi } from "@/api/weixin/wx-account";
const PAGE_SIZE_INIT = 20;
export default {
  name: "WeixinbulkmessageList",
  components: {
    WxBulkMessageListForm,
  },
  props: {},
      ...
      mainTable: {
        ...
        columnArray: [
          ...
          },
          {
            title: "群发的消息ID",
            key: "msgID",
            tableColumn: "ybj_weixinbulkmessages.MsgID",
            sortable: "custom",
          },
          {
            title: "群发状态",
            key: "status",
            tableColumn: "ybj_weixinbulkmessages.Status",
            sortable: "custom",
          },
          {
            title: "发送成功的粉丝数",
            key: "sentCount",
            tableColumn: "ybj_weixinbulkmessages.SentCount",
            sortable: "custom",
          },
          {
            title: "发送失败的粉丝数",
            key: "errorCount",
            tableColumn: "ybj_weixinbulkmessages.ErrorCount",
            sortable: "custom",
          },
          {
            title: "操作人",
            key: "userId",
            tableColumn: "ybj_weixinbulkmessages.UserId",
            sortable: "custom",
          },
          {
            title: "微信账号",
            key: "weiXinAccountId",
            tableColumn: "ybj_weixinbulkmessages.WeiXinAccountId",
            sortable: "custom",
          },
          {
            title: "是否删除",
              ...
              let actions = [
                {
                  ...
                  directives: [
                    {
                      name: "privilege",
                      value: "wx-bulk-message-list-update",
                    },
                  ],
                  action: () => {
                    this.showEditWeixinbulkmessageForm(params.row);
                  },
                },
                {
                  title: "删除",
                  directives: [
                    {
                      name: "privilege",
                      value: "wx-bulk-message-list-delete",
                    },
                  ],
                  action: () => {
                    this.$Modal.confirm({
                      ...
                      content: "确定要删除吗？",
                      onOk: () => {
                        this.deleteWeixinbulkmessage(params.row.weiXinBulkMessageId);
                      },
                    });
                  },
                },
  ...
  methods: {
    ...
    async queryList() {
      ...
      try {
        ...
        let result = await wxBulkMessageApi.queryWeixinbulkmessage(params);
    ...
    /*-------------------------删除 begin------------------------- */
    // 根据ID删除
    async deleteWeixinbulkmessage(weiXinBulkMessageId) {
      try {
        this.isShowdeleteLoading = true;
        let result = await wxBulkMessageApi.deleteWeixinbulkmessage(weiXinBulkMessageId);
    ...
    async batchDelete() {
      this.$Spin.show();
      await wxBulkMessageApi.batchDeleteWeixinbulkmessage(
        this.mainTableSelectArray.map((e) => e.weiXinBulkMessageId)
      );
    ...
    async exportAll() {
      try {
        ...
        let params = this.convertQueryParam();
        await wxBulkMessageApi.exportAll(params);
      } catch (e) {
        console.log(e);
      } finally {
    ...
    async batchExport() {
      ...
      try {
        ...
        await wxBulkMessageApi.batchExport(
          this.mainTableSelectArray.map((e) => e.weiXinBulkMessageId)
        );
      } catch (e) {
        console.log(e);
    ...
    /*-------------------------添加，修改 表单 begin------------------------- */
    //显示添加表单
    showAddWeixinbulkmessageForm() {
      this.saveModal.isUpdate = false;
      this.saveModal.show = true;
    },
    showEditWeixinbulkmessageForm(updateObject) {
