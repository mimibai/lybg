<template>
  <div>
    ...
    >
      ...
      <!-- <div class="other-way">
        <p class="inline" style="float:left">其他方式登陆(sa/123456)</p>
...
const defaultReturnUrl = config.baseUrl.defaultReturnUrl;
export default {
  name: 'LoginForm',
  props: {
    // 登录名规则
  ...
  methods: {
    ...
    async verificationCode () {
      ...
      if (data) {
        let result = await loginApi.getVerificationCode(data);
    ...
    async loginSuccess () {
      try {
        ...
        this.$store.commit('setRefreshToken', loginInfo.xrefreshToken);
        // 保存用户登录
