    ...
    >
      <div>
        ...
        >
          ...
          <Row>
            <Col span="12">
              <FormItem class="select-class" label="用户身份" prop="userIdentityId">
                <Select
                  class="form-width"
                  v-model="formValidate.userIdentityId"
                >
                  <Option
                    :key="item.value"
                    :value="item.value"
                    v-for="item in userIdentityTypes"
                    >{{ item.desc }}</Option
                  >
                </Select>
              </FormItem>
          ...
          <Row>
            ...
            <Col span="24">
              <FormItem class="select-class" label="岗位" prop="positionIdList">
                <Select
                  class="form-width"
                  multiple
                  v-model="formValidate.positionIdList"
                >
                  <Option
                    :key="item.value"
                    :value="item.value"
            ...
            <Col span="12">
              ...
              >
                <Input
                  @on-keydown="
                    formValidate.passwordAgain = formValidate.passwordAgain.replace(
                      /^ +| +$/g,
                      ''
                    )
                  "
...
import{USER_IDENTITY_TYPE} from "@/constants/user";
...
export default {
  ...
  computed: {
    userIdentityTypes: () => {
      let array = [];
      for (let item in USER_IDENTITY_TYPE) {
        let obj = {};
        obj.desc = USER_IDENTITY_TYPE[item].desc;
        obj.value = USER_IDENTITY_TYPE[item].value;
        array.push(obj);
      }
      return array;
    },
      ...
      if (typeof row === "undefined") {
        this.ruleValidate.loginPwd = [
          ...
          },
          {
            ...
            message: "密码格式错误[至少1个大写字母，1个小写字母和1个数字]",
            trigger: "blur",
          },
        ];
        this.ruleValidate.passwordAgain = [
          ...
          {
            pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^]{6,16}$/,
            message: "确认密码格式错误[至少1个大写字母，1个小写字母和1个数字]",
            trigger: "blur",
          },
        ];
      ...
      if (row && Object.keys(row).length > 0) {
        ...
        if (Object.keys(this.selectDepartment).length > 0) {
          if (this.selectDepartment.type === 1) {
            ...
            this.formValidate.organizationId = this.selectDepartment.departmentId;
            this.formValidate.organizationName = this.selectDepartment.name;
          } else {
            this.formValidate.departmentId = this.selectDepartment.departmentId;
