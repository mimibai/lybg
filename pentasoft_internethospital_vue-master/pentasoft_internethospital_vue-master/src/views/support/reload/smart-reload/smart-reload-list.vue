...
· 其他不能重启服务的应用
    ...
    >
      <Table
        :columns="mainTable.resultColumns"
        :data="mainTable.data"
        height="500"
      ></Table>
      <Page
        :current="mainTablePage.pageNum"
        :page-size="20"
...
export default {
          ...
          render: (h, param) => {
            return this.$tableAction(h, [
              ...
              },
              {
                ...
                action: () => {
                  this.mainTablePage.tag = param.row.tag;
                  this.mainTablePage.pageNum = 1;
  ...
  methods: {
    ...
    async getSmartReloadResult (tag) {
      ...
      let resultData = await smartReloadApi.getSmartReloadResult(
        tag,
        this.mainTablePage.pageNum
      );
      this.mainTable.data = resultData.data.list;
      this.mainTablePage.total = resultData.data.total;
      this.$Spin.hide();
    },
    // 修改页码
    changeMainTablePagePageNum (pageNum) {
      this.mainTablePage.pageNum = pageNum;
      this.getSmartReloadResult(this.mainTablePage.tag, pageNum);
    }
  }
};
</script>
